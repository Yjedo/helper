...
CREATE TABLE `helper_user` (
  ...
  `role_id` tinyint(4) NOT NULL COMMENT '用户权限',
...
-- 用户权限表
DROP TABLE IF EXISTS `helper_user_role`;
CREATE TABLE `helper_user_role` (
  `id` tinyint NOT NULL AUTO_INCREMENT,
  `role_name` varchar(20) NOT NULL COMMENT '角色/权限名',
  `role_level` tinyint(4) NOT NULL COMMENT '权限等级(1游客, 2普通用户, 3管理员, 4超级管理员)',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户权限表';

-- 用户权限表
DROP TABLE IF EXISTS `helper_user_status`;
CREATE TABLE `helper_user_status` (
  `id` tinyint NOT NULL AUTO_INCREMENT,
  `status_name` varchar(20) NOT NULL COMMENT '状态名',
  `status_describe` varchar(100) NOT NULL COMMENT '状态描述',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户状态表';

-- 用户明细表
DROP TABLE IF EXISTS `helper_user_detail`;
...
CREATE TABLE `helper_task`(
    ...
    `progress` tinyint(4) NOT NULL COMMENT '已完成进度',
    `is_finish` tinyint(1) NOT NULL COMMENT '任务是否完成' ,
